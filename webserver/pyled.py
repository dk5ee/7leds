...
def getframesinbackground():
    while(enableVideo):
        # todo: find better way instead of polling
        sleep(0.2)
...
<span style="background-color: %s">
    <input type="range" min="0" max="255" value="%s"
    class="slider" id="slider%s" style="width: 512px;">
</span>
...
<a href="https://github.com/dk5ee/7leds/tree/master/webserver">dk5ee 7leds</a>
    ...
    def videostreamcallback(self):
        if self.goon:
            try:
                # todo: check the linebreaks. made errors in FF
                self.wfile.write(bytes("\n--jpgboundary\n", "utf-8"))
...
if __name__ == "__main__":
    ...
    videothread = threading.Thread(
        target=getframesinbackground, name="videothread")
    videothread.start()
